...
namespace DCheatham.CodeLou.ExerciseProject
{
    ...
    {
        private List<Student> _studentList = JsonConvert.DeserializeObject<List<Student>>(System.IO.File.ReadAllText(@"..\src\StudentRecords.json"));
        ...
        public void AddStudent()
        {
            ...
            while (checkLastCompleted)
            {
                ...
                if (DateTimeOffset.TryParse(input, out var dateValue))
                {
                    if(dateValue < studentRecord.StartDate)
                    {
                    ...
                    {
                        Console.WriteLine("   Last Completed Date cannot be after Start Date, re-input date");
                    }
                }
                else
                {
